...
public final class AddHtmRefAction extends AbstractAction implements ContextAwareAction {
    ...
    private static final class ContextAction extends AbstractAction {
            ...
            task = new RequestProcessor("AddProjectBody").create(new Runnable() { // NOI18N
                ...
                public void run() {
                    ...
                    if (choosed == JFileChooser.APPROVE_OPTION) {
                        ...
                        String msg = ProjectFilter.accept(BaseUtil.managerOf(context).getServerProject(), webappFo);
                        if (msg != null) {
                        ...
                        assignHtml5ExternalServer(BaseUtil.managerOf(context).getServerProject(), selectedFile);
                    } else {
        ...
        public boolean assignHtml5ExternalServer(Project serverProject, File html5AppFile) {
            ...
            Properties html5Props = BaseUtil.loadProperties(html5AppFo.getFileObject("nbproject/project.properties"));
            ...
            ep = BaseUtil.loadEditableProperties(privateFo);
            String contextPath = BaseUtil.resolve(BaseConstants.HTML5_WEB_CONTEXT_ROOT_PROP, html5Props);
            if (contextPath == null) {
                contextPath = "/" + appName;
            }
            String urlString = BaseUtil.managerOf(context).buildUrl();
            if (urlString == null) {
                return false;
            }
            String url = BaseUtil.managerOf(context)
            ...
            if (publicFo != null) {
                EditableProperties publicProps = BaseUtil.loadEditableProperties(publicFo);
                publicProps.setProperty(SuiteConstants.HTML5_SERVER_URI_PROP, BaseUtil.getServerInstanceId(serverProject.getLookup()));
                BaseUtil.storeEditableProperties(publicProps, publicFo);
                publicFo.refresh();
            }
            ep.setProperty("server", "EXTERNAL");
            ep.setProperty("external.project.url", url);

            BaseUtil.storeEditableProperties(ep, privateFo);
    ...
    public static class ProjectFilter {
        ...
        static String accept(Project serverProject, FileObject webappFo) {
            ...
            if (webProj == null) {
                ...
                String type = BaseUtil.projectTypeByProjectXml(webappFo.getFileObject("nbproject/project.xml"));
            ...
            if (result == null) {
                ...
                if (targetFolder.getFileObject(selectedFileName) != null) {
                    ...
                    Properties props = BaseUtil.loadProperties(targetFolder.getFileObject(selectedFileName));
