...
//                    if ("spdy".equals(iniName) || "https".equals(iniName)) {
    ...
    protected void applyStartDHttpChanges(FileObject httpFo) {
        if (httpFo == null) {
            return;
        }
        HttpIni ini = new HttpIni(FileUtil.toFile(httpFo));
        String v = (String) wiz.getProperty(JettyConstants.JETTY_HTTP_PORT);
        ini.setHttpPort(v);
        ini.save();
    }
